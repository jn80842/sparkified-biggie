 ...
 * sorted SAM file.
 */
class SparkSimpleVC(reference: GenomePiece, samFile: String) extends snap.SimpleVC {
  ...
  def callSingle(pos: Int, base: Char) {
    if (base != ref(pos).toChar) {
      ...
      //println("Calling SNP " + snp + " at " + pos)
    } else {
      //snps(pos) = SNP.MATCHING(base)
      snpsset.add(SNP.MATCHING(base))
    }
  }
  ...
  def computeWeirdness(pos: Int): Float = {
    ...
    //println("weirdness for pos " + pos + " is " + weirdness + " and total coverage is " + totalCoverage)
